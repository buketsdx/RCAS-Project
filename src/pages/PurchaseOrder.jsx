...
export default function PurchaseOrder() {
  ...
  const { data: vouchers = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return all.filter(v => v.voucher_type === 'Purchase Order');
    }
  });
    ...
    { header: 'Status', accessor: 'status', render: (row) => <Badge className={row.status === 'Confirmed' ? 'bg-emerald-100 text-emerald-700' : 'bg-yellow-100 text-yellow-700'}>{row.status || 'Draft'}</Badge> },
    {
      ...
      render: (row) => (
        <div className="flex items-center gap-2">
          <Link to={createPageUrl(`PurchaseOrderForm?id=${row.id}`)}><Button variant="ghost" size="icon"><Eye className="h-4 w-4" /></Button></Link>

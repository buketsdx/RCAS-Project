...
queryFn: async () => {
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      const voucherData = { ...formData, company_id: selectedCompanyId, gross_amount: grossAmount, vat_amount: vatAmount, net_amount: netAmount };
      ...
      for (const item of items) {
        if (item.stock_item_id) {
          await rcas.entities.VoucherItem.create({
            voucher_id: voucher.id, company_id: selectedCompanyId, stock_item_id: item.stock_item_id, stock_item_name: item.stock_item_name,
    ...
    onSuccess: () => {
      ...
      window.location.href = createPageUrl('SalesOrder');
    }
  });
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <PageHeader title={voucherId ? 'Edit Sales Order' : 'New Sales Order'} backUrl="SalesOrder" />
      <form onSubmit={(e) => { e.preventDefault(); saveMutation.mutate(); }}>
        <div className="space-y-6">
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <FormField label="Delivery Date" name="due_date" type="date" value={formData.due_date} onChange={handleChange} />
          ...
          <div className="flex justify-end gap-3">
            <Button type="button" variant="outline" onClick={() => window.location.href = createPageUrl('SalesOrder')}>Cancel</Button>

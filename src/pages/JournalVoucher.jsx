...
export default function JournalVoucher() {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <PageHeader title={voucherId ? 'Edit Journal' : 'New Journal'} subtitle="Manual accounting entry" backUrl="Journal" />
        ...
        <div className="space-y-6">
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <FormField label="Journal No" name="voucher_number" value={formData.voucher_number} onChange={handleChange} placeholder="Auto" />
          ...
          <Card>
            <CardContent className="pt-6">
              <FormField label="Narration" name="narration" type="textarea" value={formData.narration} onChange={handleChange} rows={3} />
            </CardContent>
          </Card>

          <div className="flex justify-end gap-3">
            <Button type="button" variant="outline" onClick={() => window.location.href = createPageUrl('Journal')}>Cancel</Button>
            <Button type="submit" className="bg-slate-700 hover:bg-slate-800" disabled={saveMutation.isPending}>
              <Save className="h-4 w-4 mr-2" />{saveMutation.isPending ? 'Saving...' : 'Save Journal'}
            </Button>
          </div>
        </div>
      </form>

...
export default function CreditNote() {
  ...
  const { data: vouchers = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return all.filter(v => v.voucher_type === 'Credit Note' && String(v.company_id) === String(selectedCompanyId));
    },
    enabled: !!selectedCompanyId
  });

  const { data: voucherItems = [] } = useQuery({
    queryKey: ['creditNoteItems', selectedCompanyId],
    queryFn: async () => {
      ...
      render: (row) => (
        <div className="flex items-center gap-2">
          <Button variant="ghost" size="icon" asChild>
            <Link to={createPageUrl(`CreditNoteForm?id=${row.id}`)}><Pencil className="h-4 w-4" /></Link>
          </Button>
